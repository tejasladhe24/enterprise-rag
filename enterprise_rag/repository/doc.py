...
from enterprise_rag.lib.embeddings import embed_texts
...
def ingest_doc(
    ...
    with Session(engine) as session:
        ...
        chunk_texts = [chunk.text for chunk in chunk_response.chunks]
        chunk_embeddings = embed_texts(chunk_texts)

        for chunk, embedding in zip(chunk_response.chunks, chunk_embeddings):
            session.add(
                Chunk(
